...
export default async function handleProfilesRequest(req: Request<unknown, IncomingRequestCfProperties<unknown>>, db: D1Database): Promise<Response> {
  ...
  if (routePath === '' && req.method === 'GET' && profileID) {
    ...
    const allSongs = songsResults.flatMap(x => x.results);
    ...
    const allRehearsals = rehearsalsResults.flatMap(x => x.results);

    const shows = showsResult.results.map<ShowBackupInfo>(x => ({ name: x.name, songNames: allSongs.filter(song => song.show_id === x.id).map(x => x.name), noOfRehearsals: allRehearsals.filter(rehearsal => rehearsal.show_id === x.id).length }));
